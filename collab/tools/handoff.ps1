#!/usr/bin/env pwsh
# handoff.ps1 — thin PowerShell shim (path resolution only). Prefers the COLLAB_KIT_ROOT
# install-time override, else walks up for the collab-kit root signature ([C17]), then execs
# the Python core. No logic lives here ([C14]).
...
& $py (Join-Path $root 'tools/lib/handoff_cli.py') @args
exit $LASTEXITCODE
